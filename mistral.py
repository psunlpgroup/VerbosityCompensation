...
cache_dir = "/scratch2/USER/model_files/huggingface"
...
class Mistral:
    ...
    def response(self, input_example, max_new_tokens=512, do_sample=False):
        ...
        for token_scores in logits:
          ...
          output_distribution.append(list(zip(top_k_tokens, top_k_probs)))

        return result, output_distribution
...
def run(args):
  ...
  print("Loading model!")
  model = Mistral(model_name=agent_model)
    ...
    for idx, task in enumerate(tqdm(tasks)):
      ...
      while 1:
        try:
          ...
          if 'memory' in e.__str__():
              print("Memory exceeded", e)
              time.sleep(40)
          elif 'length' in e.__str__():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_file", default="./preprocessed/dataset/mmlu.json", type=str)
    parser.add_argument("--agent_model", default="mistralai/Mixtral-8x7B-Instruct-v0.1", type=str)
    parser.add_argument("--segment_length", default=3000, type=int)
    parser.add_argument("--exp_name", default='3000', type=str)
